...
| [`cultural_appropriateness`](cultural_appropriateness.md) | Judge | Language Model, Multilingual | Whether the agent respected the user's implicit cultural constraints on multilingual runs (1-3). **Opt-in** — excluded from the default run; enable via `--metrics cultural_appropriateness`. |
