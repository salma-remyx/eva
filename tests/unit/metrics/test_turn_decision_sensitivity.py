...
def test_registered_through_experience_package():
...
async def test_constant_yield_policy_scores_zero_pair_accuracy(metric):
...
async def test_constant_yield_policy_still_scores_well_on_base_turn_taking():
...
async def test_context_sensitive_agent_scores_full_pair_accuracy(metric):
...
async def test_record_without_contrast_is_skipped(metric):
...
async def test_non_english_record_is_skipped(metric):
...
async def test_events_missing_signals_are_reported_not_scored(metric):
...
def test_expected_action_backchannel_proxy(transcript: str, expected: str):
    assert TurnDecisionSensitivityMetric._expected_action(transcript) == expected


def test_decision_window_matches_turn_taking_yield_deadline():
