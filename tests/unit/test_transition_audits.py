...
def test_null_pool_measures_noise_not_effect():
...
def test_audit_keeps_real_gains_and_suppresses_phantom_ones():
...
def test_null_floor_measures_the_phantom_rate():
...
def test_thin_null_pool_is_flagged_not_guessed():
...
def test_main_writes_ledger_and_floor(tmp_path: Path):
